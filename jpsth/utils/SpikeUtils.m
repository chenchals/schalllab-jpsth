classdef SpikeUtils
    ...
    methods (Static, Access=public)
        function outArg = jpsth(spikes1, spike2, alignTime, timeWin, coinsBins)
        ...
        function outArg = alignSpikeTimes(spikes, alignTime, varargin)
            %ALIGNSPIKETIMES Returns aligned spiketime cell array
            trim = false;
            ...
            if ~iscell(spikes)
                if any(isnan(spikes))
                    ...
                    outArg = arrayfun(@(x) spikes(x,~isnan(spikes(x,:)))-alignTime(x), (1:nTrials)','UniformOutput',false);
                else
                    % assume 0 filled spiketime matrix
                    outArg = arrayfun(@(x) spikes(x,spikes(x,:)>0)-alignTime(x), (1:nTrials)','UniformOutput',false);
                end
            else
                outArg = arrayfun(@(x) spikes{x}-alignTime(x), (1:nTrials)','UniformOutput',false);
            end
            if trim
               outArg = cellfun(@(x) x(x>=timeWin(0) & x<=timeWin(1)), outArg,'UniformOutput',false);
            end
        end
