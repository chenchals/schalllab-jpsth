...
allNanRows = find(cell2mat(arrayfun(@(x) all(isnan([temp{x,:}])),(1:size(temp))','UniformOutput',false)));
temp(allNanRows,:)=[];
...
for ii = 1:nSessions
    ...
    for c = 1:size(tempUnitTable,1)
        cellInfos = [cellInfos; [tempSession tempTaskFiles  tempTaskTrials tempUnitTable(c,:)]];
    end
end
